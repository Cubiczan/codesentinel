...
function detectCircularDeps(repoInfo) {
  const liveFindings = findingsFromScan(repoInfo?.scan, "circular_deps");
  if (liveFindings) {
    return {
      type: "circular_deps",
      description: liveFindings.length
        ? "Circular dependency detection (Daytona sandbox scan)"
        : "No import cycles detected (Daytona sandbox scan)",
      findings: liveFindings,
      stats: {
        ...scanStats(repoInfo.scan),
        stronglyConnectedComponents: liveFindings.length,
        largestSCC: liveFindings[0]?.cycle?.length || 0,
      },
