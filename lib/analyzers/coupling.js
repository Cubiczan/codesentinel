...
function analyzeCoupling(repoInfo) {
  const liveFindings = findingsFromScan(repoInfo?.scan, "coupling");
  if (liveFindings) {
    return {
      type: "coupling",
      description: liveFindings.length
        ? "Coupling analysis (Daytona sandbox scan)"
        : "No high fan-out modules detected (Daytona sandbox scan)",
      findings: liveFindings,
      stats: {
        ...scanStats(repoInfo.scan),
        highFanOutModules: liveFindings.length,
      },
